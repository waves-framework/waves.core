...
using System.Linq;
using Fluid.Core.Logging.Capturing;
using Fluid.Core.Logging.Core.Enrichers;
...
namespace Fluid.Core.Logging.Core
{
    ...
    public sealed class Logger : ILogger, ILogEventSink, IDisposable
    {
        ...
        private readonly MessageTemplateProcessor _messageTemplateProcessor;
        ...
        private readonly LevelOverrideMap _overrideMap;
        ...
        public ILogger ForContext(string propertyName, object value, bool destructureObjects = false)
        {
            ...
            {
                SelfLog.WriteLine("Attempt to call ForContext() with invalid property name `{0}` (value: `{1}`)",
                    propertyName, value);
                return this;
            }
        ...
        public void Write(LogEventLevel level, string messageTemplate)
        {
            ...
            if (IsEnabled(level)) Write(level, messageTemplate, NoPropertyValues);
        }
        ...
        {
            // Avoid the array allocation and any boxing allocations when the level isn't enabled
            if (IsEnabled(level)) Write(level, messageTemplate, new object[] {propertyValue});
        }
        ...
        {
            // Avoid the array allocation and any boxing allocations when the level isn't enabled
            if (IsEnabled(level)) Write(level, messageTemplate, new object[] {propertyValue0, propertyValue1});
        }
        ...
        {
            // Avoid the array allocation and any boxing allocations when the level isn't enabled
            if (IsEnabled(level)) Write(level, exception, messageTemplate, NoPropertyValues);
        }
        ...
        {
            // Avoid the array allocation and any boxing allocations when the level isn't enabled
            if (IsEnabled(level)) Write(level, exception, messageTemplate, new object[] {propertyValue});
        }
            ...
            params object[] propertyValues)
        {
            ...
            _messageTemplateProcessor.Process(messageTemplate, propertyValues, out var parsedTemplate,
            ...
            out MessageTemplate parsedTemplate, out IEnumerable<LogEventProperty> boundProperties)
        {
            ...
            _messageTemplateProcessor.Process(messageTemplate, propertyValues, out parsedTemplate,
                out var boundEventProperties);
