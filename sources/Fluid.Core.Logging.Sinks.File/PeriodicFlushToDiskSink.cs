...
namespace Fluid.Core.Logging.Sinks.File
{
    ...
    /// </summary>
    [Obsolete(
        "This type will be removed from the public API in a future version; use `WriteTo.File(flushToDiskInterval:)` instead.")]
    public class PeriodicFlushToDiskSink : ILogEventSink, IDisposable
    {
        ...
        public PeriodicFlushToDiskSink(ILogEventSink sink, TimeSpan flushInterval)
        {
            ...
            else
            {
                ...
                SelfLog.WriteLine("{0} configured to flush {1}, but {2} not implemented",
                    typeof(PeriodicFlushToDiskSink), sink, nameof(IFlushableFileSink));
            }
        }
        ...
        private void FlushToDisk(IFlushableFileSink flushable)
        {
            ...
            {
                SelfLog.WriteLine("{0} could not flush the underlying sink to disk: {1}",
