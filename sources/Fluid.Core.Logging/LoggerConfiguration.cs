...
namespace Fluid.Core.Logging
{
    ...
    public class LoggerConfiguration
    {
        ...
        public LoggerAuditSinkConfiguration AuditTo =>
            new LoggerAuditSinkConfiguration(this, s => _auditSinks.Add(s), ApplyInheritedConfiguration);
        ...
        public Logger CreateLogger()
        {
            ...
            if (_overrides.Count != 0) overrideMap = new LevelOverrideMap(_overrides, _minimumLevel, _levelSwitch);

            var disposableSinks = _logEventSinks.Concat(_auditSinks).OfType<IDisposable>().ToArray();

            void Dispose()
            {
                foreach (var disposable in disposableSinks) disposable.Dispose();
            }
